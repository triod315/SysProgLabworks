...
namespace Task22
{
    public partial class SecondaryForm : Form
    {
        ...
        {
            string query = $"use TradeDB; update Amount_of_trade set Money = {money}, Description = n'{description}' where ID_am={id}";
            using (MySqlConnection connection = new MySqlConnection(DB))
            {
                connection.Open();
                MySqlCommand command = new MySqlCommand(query, connection);
        ...
        {
            string query = $"use TradeDB; delete from Amount_of_trade where ID_am = {id}";
            using (MySqlConnection connection = new MySqlConnection(DB))
            {
                connection.Open();
                MySqlCommand command = new MySqlCommand(query, connection);
        ...
        {
            bool res = false;
            string query = $"select * from Amount_of_trade where ID_am={id}";

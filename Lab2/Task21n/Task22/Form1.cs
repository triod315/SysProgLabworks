...
namespace Task22
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            checkDB(1, 50000, "food", 3, 458, "weapoons");
            loadAmountofTrade();
        }

        void checkDB(int id1,float money1,string description1, int id2, float money2, string description2)
        {
            string query1 = $"select * from Amount_of_trade where ID={id1} and Amount_of_trade.Money={money1} and Description='{description1}'";
            string query2 = $"select * from Amount_of_trade where ID={id2} and Amount_of_trade.Money={money2} and Description='{description2}'";
            ...
            using (SqlConnection conn = new SqlConnection(DB))
            {
                ...
                if (R.Read() == false)
                {
                    string insertQuery1 = $"use TradeDB insert into Amount_of_trade(Description, Money) values(N'{description1}', {money1})";
                    SqlCommand command1 = new SqlCommand(insertQuery1, conn);
                    
                    command1.ExecuteNonQuery();

                }
                R.Close();
                SqlCommand commandn = new SqlCommand(query2, conn);
                ...
                if (R.Read() == false)
                {
                    ...
                    SqlCommand command2 = new SqlCommand(insertQuery2, conn);
                    R.Close();
        ...
        void LoadOrgs(bool idCheck,bool amountCheck)
        {
            ...
            {
                query = "use TradeDB select * from Trading_enterprisessUkraine inner join Amount_of_trade on Amount_of_trade.ID = Trading_enterprisessUkraine.AmountOfTrade_ID";
            ...
            else
            {
                ...
                query = "use TradeDB select * from Trading_enterprisessUkraine inner join Amount_of_trade on Amount_of_trade.ID = Trading_enterprisessUkraine.AmountOfTrade_ID where ";
                if (idCheck && amountCheck)
                {
        ...
        {
            int res;
            string query1 = $"select ID from Amount_of_trade where Description=N'{description}'";
        ...
        {
            string query = $"use TradeDB delete Trading_enterprisessUkraine where ID = {id}";
            using (SqlConnection connection = new SqlConnection(DB))
            {
                connection.Open();
                SqlCommand command = new SqlCommand(query, connection);
